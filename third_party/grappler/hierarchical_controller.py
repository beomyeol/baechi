...
class HierarchicalController(Controller):
  ...
  def __init__(self, hparams, item, cluster, gpu_only=False, controller_id=0):
    ...
    """
    super(HierarchicalController, self).__init__(item, cluster, gpu_only)
  ...
  def create_op_embeddings(self, verbose=False):
    if verbose:
      tf_logging.info("process input graph for op embeddings")
    ...
    if verbose:
      tf_logging.info("num_ops = {}".format(self.num_ops))
      tf_logging.info("num_types = {}".format(len(self.type_dict)))
  ...
  def _create_group_embeddings(self, grouping_actions, child_id, verbose=False):
    ...
    """
    if verbose:
      tf_logging.info("Processing input_graph")
    ...
    if verbose:
      tf_logging.info("grouping evaluation metric")
      tf_logging.info(("num_connections={} num_intra_group_connections={} "
    ...
    if verbose:
      tf_logging.info("Finished Processing Input Graph")
  ...
  def update_reward(self,
    ...
    if verbose:
      tf_logging.info(("run_time={:<.5f} reward={:<.5f} "
                       "best_reward={:<.5f}").format(
                           run_time, reward, best_reward))
